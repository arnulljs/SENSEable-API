  ...
  return import('mqtt')
    .then(async ({ default: mqtt }) => {
      const opts = {
        ...
        // Default keepalive is 60s, which means a dropped link isn't noticed for
        // up to ~90s and the dashboard silently freezes for that whole window.
        // On a LAN bench a much shorter interval detects breaks quickly and
        // costs nothing; raise it only if the link is high-latency.
        keepalive: Number(process.env.MQTT_KEEPALIVE ?? 20),
        connectTimeout: Number(process.env.MQTT_CONNECT_TIMEOUT_MS ?? 15000),
        // Resume cleanly after a drop: we re-subscribe in the connect handler
        // below, so a clean session is the predictable choice.
        clean: true,
        resubscribe: true,
      };
